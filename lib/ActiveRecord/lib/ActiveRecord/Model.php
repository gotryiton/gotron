...
class Model
{
    ...
    protected function array_merge_keys($array_1,$array_2)
    {
        foreach($array_1 as $key => $value)
        {
            if(is_numeric($key)){
                if(array_key_exists($value,$array_2)){
                    unset($array_1[$key]);
                    $array_1[$value] = $array_2[$value];
                    unset($array_2[$value]);
                }
            }
        ...
        foreach($array_2 as $key => $value) {
            if (!array_key_exists($key, $array_1)) {
                ...
                if (is_array($value)) {
                    $array_1[$key] =  self::array_merge_keys($array_1[$key], $array_2[$key]);
                }
            }
        }
        return $array_1;
  ...
  public function to_csv(array $options=array())
  {
    ...
    public static function clear_finder_cache($name,$conditions = array(),$filters = array()){
        if(array_key_exists($name,static::$finders)){
            ...
            return \ActiveRecord\Cache::set($cache_id, md5(microtime()));
        }
        else{
            throw new Exception("Finder '$name' does not exist");
        }
