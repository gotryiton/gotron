...
class Loader
{
	...
	/**
	 * Defines the namespace used for Framework files
	 *
     * @const
	 *
	 */
 	const FRAMEWORK_NAMESPACE = "TestApp";
   ...
   /**
	...
	/**
	 * Array of paths to check for non-namespaced files
	 *
	 * @var array
	 */
    ...
    /**
     * Array of paths to check for framework files
     *
     * @var array
     */
    protected $framework_class_paths = array();

    /**
     * Array of paths to check for standard non-namespaced files
     *
     * @var array
     */
    protected $plain_class_paths = array();
    ...
    public function addPlainClassPath($path)
    {
    	$this->plain_class_paths[] = rtrim($path, DIRECTORY_SEPARATOR);
    }
     ...
     * 
     */
    public function setNamespacedPaths(array $paths = array())
    {
	...
	/**
     * 
     * Sets paths to use for non-namespaced classes. These paths are
	 * checked when there are no namespaces for a class.
     * 
     * 
     *      $loader->setClassPaths([
     *          '/path/to/zend/library',
     *          '/path/to/project/Vendor.Package/src/',
     *         	'path/to/Symfony/Component',
     *      ]);
     * 
    ...
    public function setPlainPaths(array $paths = array())
    {
        foreach ($paths as $val) {
            $this->addPlainClassPath($val);
        }
    }
    ...
    public function find($spec)
    {
        ...
        // Check the regular plain paths
        $non_namespace = str_replace($namespace, "", $spec);
        $ctf = $this->classToFile($non_namespace);
        foreach($this->plain_class_paths as $i => $path) {
            $this->tried_paths[] = "#{$i}: {$path}";
